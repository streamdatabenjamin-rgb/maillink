...
if uploaded_file:
    ...
    # ========================================
    # Manual Delete Section (Unsubscribed)
    # ========================================
    st.subheader("🗑️ Remove Unsubscribed Recipients (optional)")
    unsubscribe_check = st.checkbox("Enable manual deletion before sending")
    if unsubscribe_check:
        selected_rows = st.multiselect("Select rows to delete", df.index.tolist())
        if st.button("🗑️ Delete Selected Rows"):
            df = df.drop(selected_rows).reset_index(drop=True)
            st.success(f"✅ Deleted {len(selected_rows)} unsubscribed rows.")
