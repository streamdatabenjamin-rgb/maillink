...
def convert_bold(text):
    ...
    text = re.sub(r"\[(.*?)\]\((https?://[^\s)]+)\)", r'<a href="\2" style="color:#1a73e8; text-decoration:underline;" target="_blank">\1</a>', text)
    text = text.replace("\n", "<br>").replace("  ", "&nbsp;&nbsp;")
    return f"""
    <html>
...
if "last_saved_csv" in st.session_state:
    ...
    st.download_button("⬇️ Download Last Saved CSV", data=open(st.session_state["last_saved_csv"], "rb"), file_name=st.session_state["last_saved_name"], mime="text/csv")
...
if uploaded_file:
    ...
    df = st.data_editor(df, num_rows="dynamic", use_container_width=True, key="recipient_editor_inline", disabled=False)
...
**Your Company**""",
    ...
    delay = st.slider("Delay between emails (seconds)", min_value=20, max_value=75, value=20, step=1, help="Minimum 20 seconds delay required for safe Gmail sending.")
    ...
    def send_email_backup(service, csv_path):
        try:
            ...
            body = MIMEText("Attached is the backup CSV file for your recent mail merge run.\n\nYou can re-upload this file anytime for follow-ups.", "plain")
            msg.attach(body)
    ...
    # ========================================
    # 🚀 Send Emails / Save Drafts (Blocking with spinner & progress)
    # ========================================
    if st.button("🚀 Send Emails / Save Drafts"):
        ...
        def send_all_emails():
            ...
            if send_mode in ["🆕 New Email", "↩️ Follow-up (Reply)"]:
                ...
                with open(file_path, "rb") as f:
                    st.download_button("⬇️ Download Updated CSV", f, file_name=file_name, mime="text/csv")
        ...
        # Run inside blocking spinner
        with st.spinner("📨 Sending emails... Please do not touch the UI!"):
            send_all_emails()
